...
export default {
  ...
  async fetch() {
    ...
    const clusteredRedis = findBy(hash.clusteredRedis || [], 'id', `${namespace}/${id}`)
    const redis = findBy(hash.redis || [], 'id', `${namespace}/${id}`)
    
    this.model = clusteredRedis || redis

    if (redis) {
      this.model.spec.clusteredRedis = this.model.spec.redis
    }
  },
