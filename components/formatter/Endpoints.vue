      ...
      if ( this.value && this.value.length ) {
        ...
        try {
          ...
          out.forEach((endpoint) => {
            ...
            const port = findBy(ports, 'nodePort', endpoint.port)
            console.log(port, 'port')
            if (port) {
              endpoint.targetPort = port.targetPort
            }
          })
...
<template>
  <span>
    <template v-for="endpoint in parsed">
      ...
      >
        <span v-if="endpoint.targetPort">{{ endpoint.targetPort }}:</span><span v-if="endpoint.port">{{ endpoint.port }}/</span>{{ endpoint.protocol }}
      </a>
    </template>
  </span>
</template>
