...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      if (this.$refs.name && this.nameInputFocus) {
    ...
    <div class="row name-ns-description">
      <div v-show="!nameNsHidden" :class="{ col: true, [colSpan]: true }">
        <slot :namespaces="namespaces" name="namespace">
          ...
          />        
          <LabeledSelect 
            v-else-if="model.spec.operation === 'UPDATE' || model.spec.operation === 'DELETE'"
            v-model="name"
            :label="t(nameLabel)"
            :options="options"
            :mode="mode"
      ...
      <div v-if="!descriptionHidden" :class="{ col: true, [colSpan]: true }">
        <LabeledInput
          key="description"
          v-model="description"
          :mode="mode"
