...
export default {
  ...
  methods: {
    ...
    async save(buttonDone) {
      ...
      if (this.value.doneRoute && this.value.doneParams) {
        return this.$router.replace({
          name:   this.value.doneRoute,
          params: this.value.doneParams,
        })
      }

      this.$router.replace({
        name:   this.doneRoute,
        params: this.doneParams ? this.doneParams : { resource: this.value.type }
      })
    },
