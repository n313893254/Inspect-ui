...
export default {
  ...
  data() {
    ...
    let initialYaml
    if (this.initialForDiff) {
      initialYaml = this.createResourceYaml(this.initialForDiff)
    }
  ...
  computed: {
    ...
    isAudit() {
      ...
      return type === 'redis.cattle.io.clusteredredisaudit'
              || type === 'redis.cattle.io.redisaudit'
              || type === 'redis.cattle.io.backupaudit'
    },
  },
  ...
  methods: {
    ...
    emitOrRoute() {
      if ( this.cancelEvent ) {
        ...
        this.$router.replace({
          name:   this.doneRoute,
          params: this.doneParams ? this.doneParams : { resource }
        })
      }
    },
  ...
  watch: {
    initialForDiff(nVal) {
      if (nVal) {
        const initialYaml = this.createResourceYaml(nVal)
        this.initialYaml = initialYaml
      }
    }
  }
}
                ...
                >
                  <template #default="{checkCancel}">
                    <div class="controls-middle">
                      ...
                      <button
                        v-if="(!showPreview && isEdit) || isAudit"
                        :disabled="!canDiff"
                        type="button"
                        class="btn role-secondary"
                        @click="yamlPreview"
