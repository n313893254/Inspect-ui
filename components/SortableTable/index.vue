        ...
        <template v-for="row in group.rows">
          <slot name="main-row" :row="row">
            ...
            <tr 
              ...
              :class="{
                'main-row': true,
                'has-sub-rows': row.metadata && row.metadata.state && row.metadata.state.message || (row.status && row.status.resultMessage),
              }"
              :data-node-id="get(row,keyField)" 
              :data-cant-run-bulk-action-of-interest="actionOfInterest && !canRunBulkActionOfInterest(row)"
            >
                ...
                >
                  ...
                  >
                    <slot :name="'cell:' + col.name" :row="row" :col="col" :value="valueFor(row,col)">
                      ...
                      />
                      <template v-else-if="col.valueFor">
                        {{ col.valueFor(row,col) }}
                      </template>
          ...
          >
            <tr 
              v-if="row.metadata && row.metadata.state && row.metadata.state.message" 
              :key="row.id" 
              class="sub-row"
            >
              <td colspan="1" />
              <td :colspan="fullColspan - 1">
                <small class="text-error">
                  {{ ucFirst(row.metadata.state.message) }}
                </small>
              </td>
            </tr>
            <tr 
              v-else-if="row.status && row.status.resultMessage" 
              :key="row.id" 
              class="sub-row"
            >
              <td colspan="1" />
              <td :colspan="fullColspan - 1">
                <small class="text-error">
                  {{ ucFirst(row.status.resultMessage) }}
                </small>
              </td>
            </tr>
          </slot>
