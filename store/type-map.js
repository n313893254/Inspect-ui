...
export const getters = {
  ...
  getSpoofedInstance(state, getters) {
  ...
  allSpoofedTypes(state) {
    return Object.values(state.spoofedTypes).flat()
  },

  allSpoofedSchemas(state, getters) {
  ...
  allTypes(state, getters, rootState, rootGetters) {
    return (product, mode = ALL) => {
      ...
      if ( mode !== USED ) {
        ...
        console.log(virtualTypes, 'virtualTypes')
  ...
  hasMonitoring(state, getters) {
    return (rawType, subType) => {
      const key = getters.componentFor(rawType, subType)

      const cache = state.cache.monitoring
      ...
      try {
        require.resolve(`@/monitoring/${ key }`)
        cache[key] = true
      } catch (e) {
        cache[key] = false
      }
  ...
  importMonitoring(state, getters) {
    return (rawType, subType) => {
      const key = getters.componentFor(rawType, subType)

      return () => import(`@/monitoring/${ key }`)
    }
  },
